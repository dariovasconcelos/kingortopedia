<section class="wk-twitter wk-twitter-list">
	...
	<article>
		...
		<?php if ($show_image) : ?>
		<a class="image" href="<?php echo $tweet->getLink(); ?>" target="_blank">
		...
		<p class="meta">
			...
			<?php if ($show_author) : ?>
			<span class="author"><?php printf($this['system']->__('BY_X'), '<a target="_blank" href="'.$tweet->getLink().'">'.$tweet->name.'</a>'); ?></span>
			<?php endif; ?>
			
			<?php if ($show_date) : ?>
			<a class="statuslink" target="_blank" href="<?php echo $tweet->getStatusLink(); ?>">
